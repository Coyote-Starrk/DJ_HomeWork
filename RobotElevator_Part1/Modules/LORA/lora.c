...
uint8_t DMA_TX_BUFF[LORA_DMA_TX_SIZE];										//DMA发送缓冲
...
void Lora_Init(void)
{
  ...
  //USART 初始化设置
	//USART_InitStructure.USART_BaudRate = LORA_BAUD_RATE;															//串口波特率
...
{
	//命令设置：c0 00 07 ADDRH ADDRL CHANNEL e7 80 17 43
	DMA_TX_BUFF[0] = 0xc0;
	DMA_TX_BUFF[1] = 0x00;
	DMA_TX_BUFF[2] = 0x07;
	DMA_TX_BUFF[3] = sysTaskStatus.localAddrH;
	DMA_TX_BUFF[4] = sysTaskStatus.localAddrL;
	DMA_TX_BUFF[5] = sysTaskStatus.localChannel;
	DMA_TX_BUFF[6] = 0xe7;
	DMA_TX_BUFF[7] = 0x80;
	DMA_TX_BUFF[8] = 0x17;
	DMA_TX_BUFF[9] = 0x43;
...
void LORA_SendMsg(ADDR_ToSend addr, SYS_MsgHead resMsg, char* payload,size_t payloadLen)
{
	...
	//尽量不连续发送报文，我们用的是Lora定点发送，连续发多个不同地址的报文可能会有问题？？待验证（TODO）
...
#else
	//要求DMA缓冲区的长度要大于本次要发送的报文长度
	memcpy(DMA_TX_BUFF, &resMsg, sizeof(SYS_MsgHead));
	memcpy(DMA_TX_BUFF+sizeof(SYS_MsgHead), payload, payloadLen);
	DMA_TX_BUFF[sizeof(SYS_MsgHead)+payloadLen] = '\r';
	DMA_TX_BUFF[sizeof(SYS_MsgHead)+payloadLen+1] = '\n';

	printf("Lora reply\r\n");
	
	//设置发送长度，启动发送
	DMA_Cmd(DMA1_Channel7, DISABLE);
	DMA1_Channel7->CNDTR = sizeof(SYS_MsgHead)+payloadLen+2;
  DMA_Cmd(DMA1_Channel7, ENABLE); 
#endif
}
